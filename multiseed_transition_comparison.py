"""Multi-seed comparison of phase-transition timing for Markov vs vanilla Mamba."""
...
EVAL_EVERY = 50
PER_STEP_SECONDS = 0.22
EARLY_STOP_STREAK = 5
EARLY_STOP_STEPS = EARLY_STOP_STREAK * EVAL_EVERY
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
RESULTS_PATH = "multiseed_transition_results.json"
...
def run_single_model_experiment(
    ...
    """Train one model on the copying task and return the transition estimate."""
...
def summarize_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    model_names = ["MarkovMamba", "VanillaMamba"]
...
def main() -> None:
    ...
    estimated_seconds = total_runs * NUM_TRAIN_STEPS * PER_STEP_SECONDS
    estimated_minutes = estimated_seconds / 60
    estimated_hours = estimated_minutes / 60
    print("Multi-seed phase-transition comparison")
    print(f"Seeds: {SEEDS}")
    print(f"Runs: {total_runs} total (5 seeds × 2 models)")
    print(f"Per-step estimate: {PER_STEP_SECONDS:.2f} sec/step")
    print(
        f"Estimated total runtime for full 15000-step upper bound: "
        f"{estimated_seconds:.1f} seconds ({estimated_minutes:.1f} minutes, {estimated_hours:.2f} hours)"
    )
    print("Early stopping will shorten many runs once the model remains >= 0.99 for 250 consecutive steps.")
    ...
    for seed in SEEDS:
        ...
        print(f"{seed:4d} | {markov if markov is not None else 'NO TRANSITION':>20} | {vanilla if vanilla is not None else 'NO TRANSITION':>20} | {diff if diff is not None else 'N/A':>10}")
